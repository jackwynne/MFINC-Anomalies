...
Finally, I would like to thank my family for their support and understanding while I completed up my research. 
...
This project sets out to uncover whether the eleven common asset pricing anomalies exist because the market is correctly pricing for aggregate volatility risk. The eleven pricing anomalies are momentum, gross profitability, asset growth, investment to net assets, return on assets, net operating assets, accruals, net stock issuance, composite equity issuance, failure probability, and O-score. Even after adjusting for the three Fama-French factors, which are the market, size and value factors, the eleven pricing anomalies persist. The question this research sets out to answer is whether the anomaly returns, the abnormal return associated with the conventional anomaly portfolios, is a rational compensation of aggregated volatility risk?

To answer the research question, long-short portfolios are constructed for each of the eleven anomalies. Data from January 1986 through to December 2016 is used to construct the long-short portfolios. The returns from the long-short portfolios are regressed against a varying number of Fama and French factors to discover what is driving the returns. To these regressions, the volatility risk factor is introduced, which allows the question of whether the alpha generated by the anomaly portfolios is rational compensation for aggregate volatility risk to be answered.

After the introduction of the volatility factor to the unconditional six-factor model, none of the eleven anomaly portfolios generates any significant alpha. The lack of significant alpha after the addition of the volatility factor is the key finding of the research and supports the hypothesis that the abnormal anomaly portfolio returns are rational compensation for the aggregate volatility risk. 

To supplement the key finding, the regressions are rerun on conditional datasets that only include months that follow periods of market volatility, economic policy uncertainty and market liquidity that are in the top or bottom 30\% of values. None of the generated alphas is significant in either of the high or low market liquidity models when the volatility factor is added. The addition of the volatility factor eliminates nine out of eleven alphas in the low economic policy uncertainty model and ten out of eleven alphas in the high economic policy uncertainty and both the high and low market volatility models. These results provide supporting evidence for the hypothesis, that the abnormal returns that are generated by the anomaly portfolios are a rational compensation for aggregate volatility risk.

\hypref{sec:lit_review} of this project contains a review of three critical papers used in forming this research and a brief look at literature relevant to the pricing anomalies. \hypref{sec:data} contains a description of where the data for this research come from and how the pricing anomalies are calculated. \hypref{sec:unconditional} looks at how the anomaly portfolios perform in unconditional models from both the perspectives of excess returns and of risk-adjusted returns. The risk-adjusted analysis uses a capital asset pricing mode, Fama and French three, five and six factor models and a Fama and French six factor model with an additional volatility factor. \hypref{sec:conditional} revisits the analysis from \hypref{sec:unconditional} but this time to look at conditional data using volatility, economic policy uncertainty and market liquidity. Finally, \hypref{sec:conclusion} is a conclusion of the findings.
...
Another measure of financial distress is the O-score proposed by \citeauthoryear{ohlson1980financial}.
...
The index for economic policy uncertainty (EPU) was developed by \citeauthoryear{baker2016measuring} and is constructed from three components. The first component is an analysis of the economic sentiment of ten large US-based newspapers. The second component looks at the number of federal tax code provisions set to expire in the next ten years, based on reports from the Congressional Budget Office. The final component looks at the level of disagreement between professional economic forecasters, utilising data from the Federal Reserve Bank of Philadelphia's Survey of Professional Forecasters.

\subsubsection{Market Liquidity Data}
As a proxy for market liquidity \citeauthoryear{pastor2003liquidity}'s aggregated, liquidity index is used. 
